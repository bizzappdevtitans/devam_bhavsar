...
class HotelRoomType(models.Model):
    ...
    room_price = fields.Float(
        string="Room price", compute="_compute_prices_base_on_seasons", store=True
    )
    ...
    room_capacity = fields.Integer(
        string="Total room capacity", compute="_compute_room_capacity", store=True
    )
    ...
    def _compute_room_capacity(self):
        """compute function for default values of room capacity for a room type
        #T00471"""
        for persons in self:
    ...
    def _compute_number_of_guests(self):
        """Computes the total number of guests staying in a room #T00471"""
        for guests in self:
    ...
    def _compute_prices_base_on_seasons(self):
        """compute method to calculate room prices based on seasons  and multiply
        the room price based on its type #T00471"""
        seasons_dict = {
            "summer": {
                "start": "09-21",
                "end": "12-21",
